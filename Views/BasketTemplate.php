...
class BasketTemplate extends BaseTemplate
{
    public static function render(array $items, float $total, string $storageType): string
    {
        ...
        $title = 'Корзина — Фитнес-клуб «gym low cortisol»';
        ...
        .basket-card { border: none; border-radius: 20px; background: #ffffff; transition: transform 0.3s ease, box-shadow 0.3s ease; margin-bottom: 1rem; position: relative; overflow: hidden; }
        ...
        $noCacheMeta = '<meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate"><meta http-equiv="Pragma" content="no-cache"><meta http-equiv="Expires" content="0">';
        ...
        if (empty($items)) {
            ...
            <section class="container py-5">
                <div class="basket-empty">
                    ...
                    <h3>Корзина пуста</h3>
                    <p>Выберите клубную карту или услугу для оформления</p>
                    <a href="/products" class="btn btn-custom" style="max-width:250px">Перейти к картам</a>
                </div>
            </section>';
        } else {
            $itemsHtml = '';
            foreach ($items as $item) {
                ...
                <div class="card basket-card" id="item-' . $item['product']['id'] . '">
                    <div class="row g-0">
                        <div class="col-md-3">
                            <img src="' . htmlspecialchars($item['product']['image']) . '" class="basket-img" alt="' . htmlspecialchars($item['product']['name']) . '" onerror="this.src=\'https://placehold.co/300x200/0d6efd/ffffff?text=' . urlencode($item['product']['name']) . '\'">
                        </div>
                        <div class="col-md-6">
                            <div class="card-body p-4">
                                <h5 class="fw-bold mb-2"><a href="/product/' . $item['product']['id'] . '" class="text-decoration-none text-dark">' . htmlspecialchars($item['product']['name']) . '</a></h5>
            ...
            $content = $noCacheMeta . $customStyles . $toastContainer . '
            <section class="container py-5">
                <div class="basket-header"><h1 class="basket-title">Корзина</h1><p class="basket-subtitle">Оформление клубной карты</p></div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-lg-4">
                        <div class="basket-summary">
                            ...
                            <div class="summary-row"><span class="text-muted">Услуг:</span><span class="fw-semibold" id="summary-count">' . count($items) . '</span></div>
                            <div class="summary-row"><span class="text-muted">Общая сумма:</span><span class="fw-bold text-primary" id="summary-total">' . $totalFormatted . ' ₽</span></div>
                            <div class="basket-actions">
                                <a href="/cart/clear" class="btn btn-outline-custom" onclick="return confirm(\'Очистить корзину?\')">Очистить</a>
                                <a href="/checkout" class="btn btn-custom">Оформить карту</a>
                            </div>
                            <p class="text-muted small mt-3 mb-0 text-center">* Менеджер свяжется для активации</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            window.showBasketToast = function(title, message, type = "success") {
                ...
                const toastHTML = `<div id="${toastId}" class="toast toast-custom ${type} hide" role="alert" data-bs-delay="3000"><div class="toast-header"><strong class="me-auto fw-bold">${icons[type] || "ℹ️"} ${title}</strong><button type="button" class="btn-close" data-bs-dismiss="toast"></button></div><div class="toast-body">${message}</div></div>`;
            ...
            document.querySelectorAll(".qty-btn").forEach(btn => {
                btn.addEventListener("click", async function() {
                    ...
                    try {
                        ...
                        const response = await fetch("/cart/update", { method: "POST", headers: {"Content-Type": "application/x-www-form-urlencoded"}, body: formData });
                        ...
                        if (data && data.subtotal !== undefined) {
                            ...
                            if (data.total !== undefined) document.getElementById("summary-total").textContent = new Intl.NumberFormat("ru-RU").format(data.total) + " ₽";
